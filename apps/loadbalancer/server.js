...
/*
 * FAUX APPLICATION LOAD BALANCER — dashboard server
 * ------------------------------------------------------------------
 ...
 *   GET  /go                   302 -> the currently-active node's url
 *   GET  /go?position=2        302 -> a specific node's url
...
if (initialNode) {
  execFile(path.join(__dirname, 'switch_backend.sh'), [initialNode.url], (error) => {
    if (error) console.error(`[proxy-init] ERROR: ${error.message}`);
    else console.log(`[proxy-init] Initial backend set to ${initialNode.url}`);
  });
}
...
function setActive(target, source) {
  ...
  // Trigger the backend switch script
  execFile(path.join(__dirname, 'switch_backend.sh'), [node.url], (error, stdout, stderr) => {
    if (error) {
      console.error(`[proxy-update] ERROR: ${error.message}`);
      return;
    }
    console.log(`[proxy-update] ${stdout.trim()}`);
  });
